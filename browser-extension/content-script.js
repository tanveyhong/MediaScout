  ...
  return [...performance.getEntriesByType("resource")]
    ...
    .filter(
      (entry) =>
        ...
        /(?:\.mp4(?:[?#]|$)|mime_type=video|video_id=|douyinvod)/i.test(entry.name)
    )
  ...
  const nearby = [...document.images]
    ...
    .map((image) => {
      ...
      const overlap = mediaRect
        ? Math.max(0, Math.min(rect.right, mediaRect.right) - Math.max(rect.left, mediaRect.left)) *
          Math.max(0, Math.min(rect.bottom, mediaRect.bottom) - Math.max(rect.top, mediaRect.top))
        : 0;
      return { area: rect.width * rect.height, image, overlap };
    })
    .filter(({ area, image }) => area > 10_000 && image.complete)
    .sort((left, right) => right.overlap - left.overlap || right.area - left.area)[0]?.image;
...
function currentMediaPayload() {
  ...
  return {
    mediaUrl:
      (/^https?:/i.test(media.currentSrc || media.src || "")
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  if (message?.type === "capture-now") {
    ...
    if (payload) {
      chrome.runtime.sendMessage({ ...payload, type: "media-played" }).catch(() => {});
    }
